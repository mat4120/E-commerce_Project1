...
export const CheckoutPage = ({onClear, items, onQuantityChange, order, onAdd}) => {
            ...
            <div className='bg-neutral-900 min-h-screen py-10'>
                ...
                <div className='flex-col w-1/2 mx-auto px-15 '>
                    <div className='flex justify-between'>
                        ...
                        <button onClick={() => onClear()} className='p-2 rounded cursor-pointer bg-neutral-900 hover:bg-neutral-700 self-end transition-all duration-300'>
                    ...
                    <div className='flex flex-col justify-center gap-3 overflow-hidden flex-shrink-0'>
                        ...
                        { items.length === 0 ? (
                            <div className='flex items-center bg-neutral-600 rounded-xl'>
                                <h1 className='text-2xl font-semibold text-gray-100 p-5 flex-1'>Your cart is empty</h1>
                            </div>
                        ) :  
                        items.map((cartItem) =>(
                                <div className='flex items-center bg-neutral-600 rounded-xl'>
                                    <img src={cartItem.image_url} alt={`${cartItem.name} photo`} className='w-24 h-24 m-2 rounded-xl object-cover'></img>
                                    <h1 className='text-md text-gray-100 font-bold flex-1 '>{cartItem.name}</h1>
                                    <select 
                                        value={cartItem.quantity} 
                                        onChange={(e) => onQuantityChange(cartItem, parseInt(e.target.value))}
                                    ...
                                    <button className='mr-5 bg-red-400 px-2 rounded hover:bg-red-500 cursor-pointer transition-all duration-300' >&#xD7;</button>
                                </div>       
                        ))
                        }
                        ...
                        <div className='grid grid-cols-4 gap-1 '>
                            ...
                            {products.slice(getAlsoIndex, getAlsoIndex + 4).map((p) => (
                                <div className='bg-neutral-600 rounded-xl p-2'>
                                    <p className='font-semibold text-gray-100 text-md text-center truncate'>{p.name}</p>
                                    ...
                                    <div className="relative group cursor-pointer h-10">
                                        ...
                                        <button onClick={() => onAdd(p)} className='absolute cursor-pointer w-full bg-gray-300 text-neutral-900 px-2 py-2 font-semibold rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-300 ease-in-out'>Add To Cart</button>
                                    </div>
                                </div>
                            ))
                            }
                    ...
                    {items.length === 0 ? (
                        ...
                        <div className='flex bg-neutral-600 flex-col mt-15 p-2 rounded-xl text-gray-100'>
                            ...
                            <p className='font-bold border-b-1 border-neutral-900'>Shipping: $4.99</p>
                            <p className='text-4xl font-bold mt-2'>{`Total: $${items.reduce((acc, item) => acc + item.price * item.quantity, 0) + 4.99}`}</p>
                            <button className='bg-gray-100 text-neutral-900 p-2 rounded-b cursor-pointer hover:bg-indigo-700 w-24 transition-all duration-200' disabled={items.length === 0} onClick={() => order()}>Order</button>
                        </div>
                        )}      
                </div>
            </div>
